...
if __name__ == '__main__':
    ...
    srd=dict() #similarity (random) null distributions 
    ...
    for rep in range(1,args.Nreps+1):
        ...
        for node in g_view:
            ...
            if node in connections:#only get sims for non self connections
            ...
            for inb in range(len(connections)):
                ...
                key='d'+str(np.floor(np.log2(min(N_con_source,N_con_neighbor))))
                if key in srd:
                    srd[key].append(sim_dist[inb])
                else:
                    srd[key]=[sim_dist[inb]]

    #To improve search speed: convert lists with similarities to sorted numpy arrays.
