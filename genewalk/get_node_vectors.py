"""
This module runs DeepWalk on a network provided either via PathwayCommons, via
INDRA, or one directly provided by the user.
"""
import os
import copy
import time
...
if __name__ == '__main__':
    # TODO: change this to a CLI where it can be called as a module like
    #  python -m genewalk.get_node_vectors
    # TODO: make the documentation for the CLI appear in the documentation
    ...
    ###############This needs work: talk with Ben what is best set up
    # TODO: rename path to base_folder
    ...
    logger.addHandler(logging.FileHandler(os.path.join(args.path,'%s.log' %
                                                       logger.name)))
    ...
    if args.data_source == 'PC':
        ...
        logger.info('Currently this option is not yet available for any'
                    ' user-provided gene list, but it will become available'
                    ' for public use in the future. Choose PC as data_source'
                    ' instead to run GeneWalk with a user-provided gene list.'
                    ' Now, we proceed to demonstrate the indra option by'
                    ' running GeneWalk on the JQ1 study described in '
                    'Ietswaart et al.' )
    ...
    logger.info('total number of nodes in network %d' %
                nx.number_of_nodes(MG.graph))
